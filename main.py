...
async def paid_dalle(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if chat_id in user_state:
        if payment.checkinvoice(user_state[chat_id][1]['payment_hash']):
            if len(user_state[chat_id][0]) < 215:
                await context.bot.send_message(chat_id=chat_id,
                                               text="Generating pictures, this will take around 1 minute..")
            else:
                await context.bot.send_message(chat_id=chat_id,
                                               text="This prompt is too long for DALLE 3, the limit is 215 characters. Please click on a different AI.")
                return
            for generating in range(2):
                try:
                    file_paths = dalle3.generate_and_download(user_state[chat_id][0])
                    if isinstance(file_paths, list):
                        ...
                        await context.bot.send_message(chat_id=chat_id,
                                                       text="This request failed due to some problems with the DALLE3 "
                                                            "API.")
                        await context.bot.send_message(chat_id=chat_id,
                                                       text="You can use /refund lnbc...(invoice) with a 700 "
            ...
            await context.bot.send_message(chat_id=chat_id,
                                           text="You haven't paid, press \n/generate_dalle3 again once you paid the "
                                                "invoice")
    else:
        pass
...
async def paid_midjourney(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await context.bot.send_message(chat_id=chat_id,
                                           text="MJ is currently disabled. Click on another AI please.\n")
    return
        ...
        if user_state[chat_id][2] != True:
            ...
            if refund == "success":
                ...
                await context.bot.send_message(chat_id=chat_id, text="Your invoice is invalid, please try again with a "
                                                                     "700 Satoshi invoice. Use the format /refund "
                                                                     "bolt11Invoice")
            elif refund == "error":
...
if __name__ == '__main__':
    ...
    payment_handler_dalle = CommandHandler('generate_dalle3', paid_dalle)
