...
def connect():
    count = 0
    try:
        relay_manager.add_relay("wss://relay.grunch.dev")
            ...
            if event_msg.event.kind == 42:
                if event_msg.event.content[0:3] == "/p ":
                    ...
                    event = Event(public_key,
                                  "Send /gd (DALLE2) or /gsd (Stable Diffusion) or /gmj (Midjourney like, experimental, 2 pics) once you paid the invoice to start generating",
                                  kind=42,
                                  tags=[["e", os.environ['nostr_chat_id']]], created_at=int(time.time()))
                    event.sign(private_key)
                    message_2 = json.dumps([ClientMessageType.EVENT, event.to_json_object()])
                    ...
                    if payment.checkinvoice(user_state_nostr[current_prompt]['payment_hash']):
                        ...
                        event = Event(public_key,
                                      "You havent paid yet, send /gd again once you paid or give a new prompt with /p",
                                      kind=42,
                                      tags=[["e", os.environ['nostr_chat_id']]], created_at=int(time.time()))
                        event.sign(private_key)
                        message_2 = json.dumps([ClientMessageType.EVENT, event.to_json_object()])
                ...
                if content[0:3] == "/p ":
                    ...
                    user_state_nostr[current_prompt] = payment.getinvoice()
                    event = Event(public_key,
                                  encrypt_message(str("https://api.qrserver.com/v1/create-qr-code/?size=300x300&data=" +
                                                      user_state_nostr[current_prompt][
                                                          'payment_request'] + "&format=.png") + " " +
                    ...
                    if payment.checkinvoice(user_state_nostr[current_prompt]['payment_hash']):
                        ...
                        event = Event(public_key, encrypt_message(
                            "You havent paid yet, send /gd again once you paid or give a new prompt with /p", ss),
                                      kind=4,
                                      tags=[["p", user_pk]], created_at=int(time.time()))
                        event.sign(private_key)
                        message_2 = json.dumps([ClientMessageType.EVENT, event.to_json_object()])
    ...
    if message_type == 42:
        ...
        if images == "failure":
            ...
            for image in images:
                ...
                link = list(rc.link('dropbox:lpb/' + m))
                link[-2] = '1'
                event = Event(public_key, encrypt_message(''.join(link) + " " + prompt + ", Midjourney model", ss),
                              kind=4,
                              tags=[["p", user_pubk]], created_at=int(time.time()))
                event.sign(private_key)
                message_2 = json.dumps([ClientMessageType.EVENT, event.to_json_object()])
...
def dalle_generate(prompt, type, user_pk):
    ...
    if type == 42:
        if generations == "violation":
            ...
            event = Event(public_key,
                          "Your prompt violated the OpenAI terms so it got blocked by them. Try to avoid controversial stuff or use Stable Diffusion with /gsd",
                          kind=42,
                          tags=[["e", os.environ['nostr_chat_id']]], created_at=int(time.time()))
            event.sign(private_key)
            message_2 = json.dumps([ClientMessageType.EVENT, event.to_json_object()])
        ...
        if generations == "violation":
            ...
            logging.error("dalle error nostr: " + prompt)
            event = Event(public_key, encrypt_message(
                "Whoops, this failed. Sometimes the Dalle API is unreliable, send an invoice and "
                "@f321x will try to refund you. You can try again later.", ss), kind=4,
                          tags=[["p", user_pk]], created_at=int(time.time()))
            event.sign(private_key)
            message_2 = json.dumps([ClientMessageType.EVENT, event.to_json_object()])
...
def sd_generate(prompt, type, user_pk):
    ...
    if type == 42:
        ...
        for generating in range(2):
            if stablediffusion.find_seed(prompt) == "seed_too_long":
                event = Event(public_key,
                              encrypt_message("Seed too long, use max. 9 digits, generating without seed now.", ss),
                              kind=4,
                              tags=[["p", user_pk]], created_at=int(time.time()))
                event.sign(private_key)
                message_2 = json.dumps([ClientMessageType.EVENT, event.to_json_object()])
                ...
                event = Event(public_key,
                              encrypt_message("Seed not an integer (number), generating without seed now.", ss), kind=4,
            ...
            try:
                ...
                for guidance in range(7, 11):
                    ...
                    link = list(rc.link('dropbox:lpb/' + 'sd_picture_gd_' + str(guidance) + "_" + id + '.png'))
                    link[-2] = '1'
                    event = Event(public_key, encrypt_message(''.join(link) + " " + prompt + ", Stable Diffusion", ss),
                                  kind=4,
                                  tags=[["p", user_pk]], created_at=int(time.time()))
                    event.sign(private_key)
                    message_2 = json.dumps([ClientMessageType.EVENT, event.to_json_object()])
                ...
                if generating == 0:
                    logging.error("nostr sd error: " + prompt)
                    event = Event(public_key, encrypt_message(
                        "Failed, trying again. If it doesn't give you pictures in a minute click "
